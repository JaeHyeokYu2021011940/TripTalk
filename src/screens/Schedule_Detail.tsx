...
import { StyleSheet, View, Text, ImageBackground, Image, TouchableOpacity, ScrollView, Platform, PanResponder, Animated, Dimensions } from "react-native";
...
<script>
    ...
    kakao.maps.load(function() {
        ...
        var positions = [
            { label: '1', title: '광화문', lat: 37.5759, lng: 126.9768 },
            { label: '2', title: '메가MGC커피 광화문점', lat: 37.5710, lng: 126.9745 },
            { label: '3', title: '세종문화회관 예술의정원', lat: 37.5721, lng: 126.9760 },
            { label: '4', title: '청진공원', lat: 37.5710, lng: 126.9800 }
        ];
...
const MAP_MAX_HEIGHT_LIMIT = SCREEN_HEIGHT - HEADER_HEIGHT - TABBAR_HEIGHT - TIMELINE_TITLE_AREA;
const MAP_MIN_HEIGHT_LIMIT = -40; // Allow it to hide completely under the header

const ScheduleDetail = ({ onNavigate }: { onNavigate: (screen: string) => void }) => {
	...
	const panResponder = React.useRef(
		PanResponder.create({
			...
			onPanResponderMove: (e, gestureState) => {
				...
				if (newHeight >= MAP_MIN_HEIGHT_LIMIT && newHeight <= MAP_MAX_HEIGHT_LIMIT) {
			...
			onPanResponderRelease: (e, gestureState) => {
				...
				if (lastMapHeight.current < MAP_MIN_HEIGHT_LIMIT) lastMapHeight.current = MAP_MIN_HEIGHT_LIMIT;
				if (lastMapHeight.current > MAP_MAX_HEIGHT_LIMIT) lastMapHeight.current = MAP_MAX_HEIGHT_LIMIT;
			},
		})
	).current;
	...
	return (
		<View style={[styles.scheduleDetail, styles.icon17Layout]}>
			<View style={styles.header}>
				<TouchableOpacity style={styles.button9} onPress={() => onNavigate('Schedule')}>
					<View style={[styles.icon17, styles.icon17Layout]}>
						<Image style={styles.icon18} resizeMode="cover" source={Icons.BackL} />
					</View>
				</TouchableOpacity>
				<View style={styles.title}>
					<Text style={styles.titleText}>서울 여행</Text>
			...
			<View style={styles.stickyHeader}>
				...
				<View style={[styles.frame2, styles.frame2SpaceBlock]}>
					<Text style={[styles.text7, styles.textFlexBox]}>내 일정 타임라인</Text>
				...
				<View style={[styles.frame4, styles.frame4Border]}>
					<View style={[styles.button2, styles.buttonFlexBox]}>
						<Text style={[styles.text8, styles.textLayout]}>1일차</Text>
					</View>
					<View style={styles.buttonFlexBox}>
						<Text style={[styles.text9, styles.textLayout]}>2일차</Text>
					</View>
					<View style={styles.buttonFlexBox}>
						<Text style={[styles.text9, styles.textLayout]}>3일차</Text>
					</View>
				</View>
			</View>
